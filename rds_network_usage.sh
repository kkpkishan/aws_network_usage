...
INSTANCES="${ADD_INSTANCES} $(aws rds describe-db-instances --region ${REGION} --query DBInstances[*].DBInstanceIdentifier --output text)" || { echo "Failed to run aws rds describe-db-instances commandline, exiting..."; exit 1; }
...
for _instance_id in ${INSTANCES}; do
	unset _value
	_value="$(aws cloudwatch get-metric-statistics --metric-name ${METRIC} --start-time ${START_TIME} --end-time ${END_TIME} --period 86400 --namespace AWS/RDS --statistics Sum --dimensions Name=DBInstanceIdentifier,Value=${_instance_id} --region ${REGION} --output text)"
	...
	echo "${_instance_id}: $(echo "${_value}" | awk '{ sum += $2 } END {printf ("%f\n", sum/1024/1024/1024)}';) GiB";
        echo "${_value}" | awk '{ sum += $2 } END {printf ("%f\n", sum/1024/1024/1024)}' >> tmp 
done
echo "Total: $(awk '{ sum += $1 } END { print sum }' tmp) GiB"
        rm -rf tmp
